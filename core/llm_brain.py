...
class LLMBrain:
    ...
    def _build_system_prompt(self):
        tools_desc = registry.get_available_descriptions()
...
2. ACTION: 调用工具。格式必须是独立一行: ACTION: FUNCTION_NAME(param1="value", param2="value")
   - 每次只能调用一个工具 (ONE action per turn)。
   - 严格遵守 Python 函数调用语法。
   - ACTION: 后面只能跟函数调用，不能有其他文字。
   - 注意：不要在 ACTION 行末尾追加 PAUSE 等文字，PAUSE 必须单独一行。
3. PAUSE (单独一行，表示等待工具执行结果)
4. ANSWER: 给用户的最终回复。如果你需要向用户提问、要求提供更多信息（如时间不明）或确认操作，请直接在此处输出问题，以等待用户下一步回答！严禁调用不存在的问询工具（如 ask_user 等）。

重要: 只调用上面「可用工具」列出的工具。如果某个工具没有列出，说明它当前不可用，请勿尝试调用。
...
"""
    ...
    def _clean_action_line(self, action_line):
        """清理 ACTION 行，去掉尾部 PAUSE 等多余文字，只提取纯函数调用。
        
        模型经常输出 'get_calendar_events(...)PAUSE' 或 'activate_focus_mode() PAUSE'
        这会导致 ast.parse 失败。用正则提取 func_name(...) 部分。
        """
        # 先去掉常见的尾部垃圾：PAUSE, pause 等
        action_line = re.sub(r'\s*PAUSE\s*$', '', action_line, flags=re.IGNORECASE)
        action_line = action_line.strip()
        
        # 尝试直接 ast.parse，如果成功就直接返回
        try:
        ...
        # ast.parse 失败，用正则提取函数调用 func_name(...)
        # 匹配 func_name( 然后贪婪匹配到最后一个 )
        match = re.search(r'([a-zA-Z_]\w*\(.*\))', action_line, re.DOTALL)
        if match:
            return match.group(1).strip()
        
        return action_line

    def _extract_bare_tool_call(self, content):
        """检测内容中是否有未加 ACTION: 前缀的已注册工具调用。
        
        例如 LLM 输出 'THOUGHT: ... activate_focus_mode()' 没有 ACTION: 标签。
        """
        ...
        for step in range(max_steps):
            ...
            if "ACTION:" in content:
                try:
                    ...
                    # 清理 action_line：去掉尾部 PAUSE 和多余文字，只保留函数调用
                    action_line = self._clean_action_line(action_line)
                    if not action_line: continue
                ...
                # 既没有 ACTION 也没有严格的 ANSWER:，检查是否有裸函数调用
                bare_call = self._extract_bare_tool_call(content)
                if bare_call:
                    # 当作 ACTION 执行
                    if step_callback: step_callback("action_call", bare_call)
                    try:
                        tree = ast.parse(bare_call)
                        if tree.body and isinstance(tree.body[0], ast.Expr) and isinstance(tree.body[0].value, ast.Call):
                            call_node = tree.body[0].value
                            func_name = call_node.func.id if isinstance(call_node.func, ast.Name) else call_node.func.attr
                            tool_func = registry.get_tool(func_name)
                            if tool_func:
                                kwargs = {}
                                for kw in call_node.keywords:
                                ...
                                try:
                                    ...
                                    tool_result = f"Runtime Error: {e}"
                                print(f"🔧 Bare call executed: {func_name} -> {tool_result}")
                                self.history.append({"role": "user", "content": f"TOOL_OUTPUT: {tool_result}"})
                                if step_callback: step_callback("observation", tool_result)
